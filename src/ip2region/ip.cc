...
ip_t::ip_t(const char* p) {
    from_xdb(p, ip_version, ip_size);
}

bool ip_t::from_str(const string& str, int version) {
    int af_inet = (version == ipv4) ? AF_INET : AF_INET6;
    return inet_pton(af_inet, str.data(), p) == 1;
}

void ip_t::from_xdb(const char str[16], int version, int size) {
    memset(p, 0, 16);
...
node_t::node_t(char* buf) {
    ...
    *pos2 = '\0';
    ...
    if (!ip1.from_str(buf, ip_version) || !ip2.from_str(pos1 + 1, ip_version) || ip2 < ip1 ||region.empty()) {
        *pos1 = *pos2 = '|';
